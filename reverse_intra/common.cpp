...
int client::send(std::string savefilename)
{
    ...
    for (req_id = 0; req_id < req_num; req_id++)
    {
        ...
        std::cout << wrs[req_id].opcode << ", " << wrs[req_id].num_sge << ", " << wrs[req_id].sg_list[0].addr << ", " << wrs[req_id].sg_list[0].length << ", " << wrs[req_id].sg_list[0].lkey << ", " << wrs[req_id].wr.rdma.remote_addr << ", " << wrs[req_id].wr.rdma.rkey << std::endl;
    }
...
//             {
//                 time_this_ns = std::chrono::time_point_cast<std::chrono::nanoseconds>(std::chrono::high_resolution_clock::now()).time_since_epoch().count();

// #if OUTPUT_FILE == 1
//                 save_file << time_this_ns << " " << req_send_cnt << " " << req_comp_cnt << std::endl;
// #else
//                 if(time_last_ns != 0)
//                 {
