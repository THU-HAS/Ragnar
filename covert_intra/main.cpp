...
int main(int argc, char* argv[]) {
    ...
    std::cout << "static flow :" << std::endl;
    std::vector<request_t> reqs = readRequestsFromFile(file);
    std::cout << "dynamic flow - bit 0 :" << std::endl;
    std::vector<request_t> reqs_bit0 = readRequestsFromFile(file);
    std::cout << "dynamic flow - bit 1 :" << std::endl;
    std::vector<request_t> reqs_bit1 = readRequestsFromFile(file);

    config.req_num = (uint)reqs.size();
    config.req_num_bit0 = (uint)reqs_bit0.size();
    config.req_num_bit1 = (uint)reqs_bit1.size();
    config.reqs = reqs;
    config.reqs_bit0 = reqs_bit0;
    config.reqs_bit1 = reqs_bit1;
