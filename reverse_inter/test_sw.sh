...
for ((i=1;i<=$f1;i++))
do
    ib_send_bw -d $dev1  -s $s1 -q $q1 -p $[$i+4000] --run_infinitely -F --tclass $tos1 >/dev/null &
done

for ((i=1;i<=$f2;i++))
do
    # ib_write_bw -d $dev2  -s $s2 -q $q2 -p $[$i+5000] --run_infinitely -F --tclass $tos2 >/dev/null &
    ssh vertigo "ib_send_bw -d $dev2  -s $s2 -q $q2 -p $[$i+5000] --run_infinitely -F --tclass $tos2 >/dev/null &"
done
